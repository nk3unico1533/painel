// Dark Aurora — API PROXY DEFINITIVO (Render Safe)
// Funciona no Node 18+ SEM node-fetch
...
// Função padrão de proxy usando fetch NATIVO do Node
async function proxy(res, url) {
  try {
    const r = await fetch(url); // <= agora nativo
...
app.listen(PORT, () => {
  console.log("Dark Aurora — API Proxy ONLINE ✔ (Render Mode)");
});